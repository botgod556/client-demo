...
class GPIOMonitor:
    ...
    def pin_pressed(self, pin):
        ...
        # Calculate time difference (how long it was HIGH/released)
        time_diff_s = (current_time - self.pin_timestamps[pin])
        
        logger.info(f"Pin {pin} changed to LOW (pressed), was HIGH for {time_diff_s:.2f}ms")
        
        # Send data to server
        self.send_data_to_server(pin, False, time_diff_s)  # False = LOW
    ...
    def pin_released(self, pin):
        ...
        # Calculate time difference (how long it was LOW/pressed)
        time_diff_s = (current_time - self.pin_timestamps[pin])
        
        logger.info(f"Pin {pin} changed to HIGH (released), was LOW for {time_diff_s:.2f}ms")
        
        # Send data to server
        self.send_data_to_server(pin, True, time_diff_s)  # True = HIGH
        
        # Update state and timestamp
        self.pin_states[pin] = True  # HIGH
        self.pin_timestamps[pin] = current_time
    
    def send_data_to_server(self, pin, state, time_diff_s):
        """Send pin change data to the server"""
        data = {
            'device_name': self.device_name,
            'pin': pin,
            'state': 'HIGH' if state else 'LOW',
            'time_diff_ms': round(time_diff_s, 2),
