...
class GPIOMonitor:
    ...
    def pin_pressed(self, pin):
        ...
        logger.info(f"Pin {pin} changed to LOW (pressed), was HIGH for {time_diff_ms:.2f}ms")
        
        # Send data to server
        self.send_data_to_server(pin, False, time_diff_ms)  # False = LOW
    ...
    def pin_released(self, pin):
        ...
        logger.info(f"Pin {pin} changed to HIGH (released), was LOW for {time_diff_ms:.2f}ms")
        
        # Send data to server
        self.send_data_to_server(pin, True, time_diff_ms)  # True = HIGH
